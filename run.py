...
def _detect_strongest_corners_per_region(gray_f32):
    h, w = gray_f32.shape
    regions = [
...
def _draw_results(vis, results, scale_x, scale_y):
...
def _build_trapezoid_object_points():
...
def _build_camera_matrix(width, height):
...
def _rotation_matrix_to_euler_zyx_deg(rot_mat):
...
def _solve_pose(results, width, height):
    img_pts = _get_image_points(results)
    if img_pts is None:
        return None

    obj_pts = _build_trapezoid_object_points()
    ...
    roll_deg, pitch_deg, yaw_deg = _rotation_matrix_to_euler_zyx_deg(rot_mat)
...
def _check_tilt_and_extract_values(
    gray_f32,
    angle_abs_deg_max=ANGLE_ABS_DEG_MAX,
    xy_abs_mm_max=XY_ABS_MM_MAX,
    z_mm_min=Z_MM_MIN,
    z_mm_max=Z_MM_MAX,
):
